    ...
    return new Promise(function (resolve, reject) {
      if (typeof show === 'string' || typeof show === 'number') {
        ...
        models.Episode
          ...
          .then(function () {
            var episodesPromise = [];
            if (Array.isArray(episodes) === false) {
              if (!episodes.EpisodeName) {
                episodes.EpisodeName = 'TBA';
              }
              episodesPromise.push(insertEpisode(show.id, episodes));
            } else {
              episodesPromise = episodes.map(function (episode) {
